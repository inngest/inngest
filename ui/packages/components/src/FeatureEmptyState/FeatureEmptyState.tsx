...
export type FeatureEmptyStateProps = {
  ...
  docsUrl: string;
  // Tracking is injected so each app can wire its own analytics pipeline
  // (Segment in the dashboard, /v0/telemetry in the dev server).
  onView?: () => void;
  onPromptCopy?: () => void;
  onExampleCopy?: () => void;
  ...
  // started" section) in the full layout while keeping the title,
  // description, and docs link. Used by the dev server's fake-door pages
  // to show a CTA card to users who have already used the feature.
...
function ExampleBlock({
  ...
  height,
  onExampleCopy,
}: FeatureEmptyStateProps['example'] & { onExampleCopy?: () => void }) {
  ...
  return (
    <CommandBlock.Wrapper>
      ...
      >
        ...
        <CommandBlock.CopyButton content={current?.content} onCopy={() => onExampleCopy?.()} />
      </CommandBlock.Header>
