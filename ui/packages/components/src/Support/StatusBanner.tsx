...
import { impactSchema, type Impact } from '@inngest/components/SharedContext/useInngestStatus';
import {
  RiAlertFill,
  RiArrowDownSLine,
...
export function StatusBanner({ status }: StatusBannerProps) {
  ...
  return (
    <div
      className={`flex w-full flex-col items-start gap-2 px-4 py-2 ${
        isOutage
          ? 'bg-error text-error'
          : isMaintenance
          ? 'bg-info text-info'
    ...
    >
      <div className="flex w-full flex-1 items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {isMaintenanceScheduled && <p className="text-sm">Upcoming maintenance scheduled</p>}
        </div>
        <button
        ...
        >
          <RiArrowDownSLine className={`text-muted h-4 w-4 ${isOpen ? 'rotate-180' : ''}`} />
        </button>
      </div>
      <div className={`flex flex-col items-start gap-3 pb-1 pt-2 ${isOpen ? 'block' : 'hidden'}`}>
        {status.incidents.length > 0 &&
          status.incidents.map((incident) => <Event key={incident.id} event={incident} />)}
