  ...
  ({ children, className, ...props }, forwardRef) => {
    return (
      <button
        ...
        ref={forwardRef}
        className={cn(
          'border-muted bg-surfaceBase text-basis group box-content flex h-[32px] w-full items-center justify-between rounded-l-[5px] rounded-r-[5px] border px-2 text-sm',
          className
        )}
      >
        {children}
