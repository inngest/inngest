...
ORDER BY
  ...
  },
  {
    ...
    query: `WITH
    -- For event-received-to-run-start latency, input.ts approximates receipt
    -- time: it defaults to server receipt time unless the sender provides event.ts.
    -- started_at - fromUnixTimestamp64Milli(toUInt64(input.ts)) AS function_start_latency
    started_at - queued_at AS function_start_latency
SELECT
    function_id,
    toStartOfInterval(queued_at, INTERVAL 5 MINUTE) AS bucket,
...
ORDER BY
    ...
    function_id`,
    explanation:
      'Analyze queue-to-start latency every 5 minutes over the past hour.',
    templateKind: 'time',
  },
];
