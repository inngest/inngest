...
export const formatDimension = (param: any) => {
  ...
  const bgColor = window.chartSelected === param.seriesIndex ? `background-color:${color}40` : '';

  //
  // FYI using vanilla html in formatter because rendering react here causes
  // some lag with synched cursors
  return `<div class="flex flex-row justify-between flex-nowrap items-center px-2" style="${bgColor}">
    <div>
...
const tooltipFormatter: any = (params: any) => {
  const sorted = [
    ...params.filter((p: any) => p.seriesIndex === window.chartSelected),
    ...params
      .filter((p: any) => p.seriesIndex !== window.chartSelected)
      .sort((a: any, b: any) => b.value - a.value),
  ];
  return `<div class="my-1"><div class="mb-1 mx-2 text-sm">${params[0].axisValue}</div>${sorted
