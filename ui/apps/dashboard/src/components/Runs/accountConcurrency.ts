...
// How far behind "now" the window ends. ClickHouse ingestion typically trails
// by around a minute, and the metrics query zero-fills gaps for counters
// (WITH FILL, with INTERPOLATE applied to gauges only), so a not-yet-ingested
// minute is indistinguishable from a minute with no limit hits. Ending the
// window before the lag horizon keeps the freshest, least trustworthy bucket
// out of the sample instead of silently counting it as a zero.
export const CONCURRENCY_LAG_OFFSET_MS = 60 * 1000;
 ...
 * ending CONCURRENCY_LAG_OFFSET_MS before now so we never read a bucket
 * ClickHouse may not have finished ingesting.
 *
...
export function buildConcurrencyWindow(now: number): ConcurrencyWindow {
  const end = floorToMinute(now) - CONCURRENCY_LAG_OFFSET_MS;
