 ...
 * limit, which is the usual explanation for runs sitting queued longer than
 * expected. Renders nothing when there's no pressure or the user dismissed it
 * within the last day.
 */
type Props = {
...
export function AccountConcurrencyBanner({ refreshNonce, scope }: Props) {
  ...
  // Deduped per account per session, since the banner re-resolves on every
  // mount and every Refresh.
  useEffect(() => {
    if (!isVisible || !accountID || hasTrackedView(accountID)) {
      return;
    }
    markViewTracked(accountID);
    trackBannerViewed({
