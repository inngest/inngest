...
describe('scanProgressivePages', () => {
  ...
  it('collects another display target when resuming with existing matches', async () => {
    const fetchPage = vi
      .fn()
      .mockResolvedValueOnce({ items: [], cursor: 'cursor-2', hasMore: true })
      .mockResolvedValueOnce({
    ...
    const reason = await scanProgressivePages<{ id: string }>({
      ...
      initialItems: [{ id: 'first' }, { id: 'second' }],
      fetchPage,
      onCommit: () => {},
      signal: new AbortController().signal,
      displayTarget: 2,
    ...
    expect(reason).toBe('display-target');
