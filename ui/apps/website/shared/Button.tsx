...
export function Button({
  ...
  return (
    <a
      target={target}
      href={props.href}
      className={`button group inline-flex items-center justify-center gap-0.5 whitespace-nowrap rounded-lg font-medium tracking-tight transition-all ${variants[variant]} ${sizes[size]} ${props.className} ${width}`}
    >
      {arrow && arrow === 'left' ? (
        <ArrowRight className="-ml-1.5 rotate-180 transition-transform duration-150 group-hover:-translate-x-1" />
      ) : null}
      {children}
      {arrow && arrow === 'right' ? (
