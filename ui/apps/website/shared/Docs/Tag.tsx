...
const colorStyle = (color: string, variant: string, background: 'default' | 'page'): string => {
  switch (variant) {
    case 'small':
      return `text-${color}-${background === 'default' ? '200' : '600'} dark:text-${color}-${
        background === 'default' ? '300' : '200'
      }`;
