...
export function Guide(guide: { href: string; name: string; description: string }) {
  return (
    <div key={guide.href}>
...
export function GuideGrid({ cols = 4, children }) {
  return (
    <div
      className={`not-prose mt-4 grid grid-cols-1 gap-8 border-t border-slate-900/5 pt-10 sm:grid-cols-2 xl:max-w-none
      dark:border-white/5 xl:grid-cols-${cols}`}
    >
      {children}
    </div>
  );
