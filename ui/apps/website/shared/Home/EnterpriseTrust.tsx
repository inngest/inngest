...
export default function EnterpriseTrust() {
  return (
    <Container className="mt-12">
      <Heading title="Scale with confidence" lede={<></>} className="text-center" />

      <div className="mx-auto my-16 grid max-w-6xl md:grid-cols-3">
        {numbers.map(({ title, subtitle }) => (
          <h3 className="flex flex-col items-center gap-2 " key={title}>
            <span
              className="bg-gradient-to-br from-white to-slate-300 bg-clip-text text-7xl font-extrabold tracking-tight text-transparent"
              style={
                {
