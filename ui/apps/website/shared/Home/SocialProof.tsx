...
const quotes = [
  {
    ...
    quote: (
      <>
        For anyone who is building multi-step AI agents (e.g AutoGPT type systems), I highly
        recommend building it on top of a job queue orchestration framework like <AtInngest />, the
        traceability these things provide out of the box is super useful, plus you get timeouts &
        retries for free.
      </>
    ),
    name: 'David',
  ...
  },
  {
    ...
    quote: (
      <>
        ...
        <br />
        <br />
        This function potentially runs for a long time and this allows us to not run this function
        again when the previous function hasn't finished based on the combination specified in
        'key'.
        <Image
  ...
  },
  {
    ...
    quote: (
      <>
        I love this product so much! I spent 2 days setting up some background workers on Render.com
        and it was a total pain in the ass. I gave up and I got my background jobs set up in under
        10 minutes with Inngest.
      </>
    ),
    name: 'Ray Amjad',
    username: 'theramjad',
  ...
  },
  {
    ...
    quote: (
      <>
        Yeh so <AtInngest /> is perhaps one of the best SaaS platforms I have EVER used, incredible
        stability and crystal clear APIs. Love it already!
      </>
    ),
  ...
  },
  {
    ...
    quote: (
      <>
        The trickiest part was handling large background jobs in a serverless infrastructure.{' '}
        <AtInngest /> was key to allow us synchronize all your bank transactions to Notion
        seamlessly.
      </>
    ),
    name: 'Ivan Garcia',
  ...
  },
  {
    ...
    quote: (
      <>
        Just came across <AtInngest />. This looks bloody gorgeous! Can't wait to find an idea to
        plug this in.
        <br />
        <br />
  ...
  },
  {
    ...
    quote: (
      <>
        ok, <AtInngest /> is incredible... really clear messaging, great docs, fast and well
  ...
  },
  {
    ...
    quote: (
      <>
        As someone who used to Promise.all and pray I am happy tools like <AtInngest /> exist.
      </>
    ),
    name: 'David parks',
    username: 'dparksdev',
...
export default function SocialProof({ className }: { className?: string }) {
  return (
    <Container className={`relative z-30 my-44 ${className}`}>
      <Heading title="What developers are saying" className="mx-auto max-w-2xl text-center" />
      <div className="mt-16 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        {quotes.map(({ name, username, quote, avatar }) => (
          <div className="mx-auto flex max-w-[420px] flex-col gap-4 rounded-md border border-slate-500/10 bg-slate-900/80 p-6">
            <div className="flex w-full flex-row items-center gap-4 font-medium">
              ...
              <span className="grow text-sm">
                {name}
                {!!username && <span className="ml-2 text-slate-600">@{username}</span>}
              </span>
            </div>
