...
export const getTicketById = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<TicketDetail | null> => {
    try {
      const { ticketId } = data;
      const userEmail = await requireAuthenticatedEmail();
...
// Create ticket types
export type CreatePlainThreadInput = {
  user: {
    id: string;
    name?: string;
  };
...
export async function createPlainThread({
  user,
  ticket,
}: CreatePlainThreadInput): Promise<CreateTicketResult> {
  const authEmail = await requireAuthenticatedEmail();
  ...
  const threadInput: {
    ...
    title: ticket.title || ticketTypeTitles[ticket.type] || "Support request",
    components: [
      {
        componentText: {
          text: ticket.body,
...
export const createTicket = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<CreateTicketResult> => {
    try {
      return await createPlainThread(data);
    } catch (error) {
...
export const getCustomerTierByEmail = createServerFn({ method: "GET" }).handler(
  async (): Promise<CustomerTierInfo> => {
    try {
      const email = await requireAuthenticatedEmail();
