...
test.describe('Dev Server UI - Configuration Test', () => {
  ...
  test('Browser capabilities work', async ({ page }) => {
    await page.goto(
      'data:text/html,<html><body><h1>Dev Server Test</h1></body></html>',
    );

    // Test basic selectors and assertions work
    await expect(page.locator('h1')).toHaveText('Dev Server Test');
  ...
  test('Environment variables are loaded', async ({ page }) => {
    ...
    expect(process.env.INNGEST_DEV).toBe('true');
  });
});
