...
function CellValueDisplay({
  ...
  const { content, language, monacoLinks } = useMemo(() => {
    ...
    // A column typed something other than Json (e.g. Unknown, for a
    // VARIANT column the backend can't statically type) can still carry a
    // real object or array value at runtime -- String(value) on those
    // produces "[object Object]"/a bare comma-joined list rather than
    // useful content, so route them through the same JSON rendering a
    // Json-typed column gets (see InsightsQueryTab.tsx's renderCell for
    // the equivalent fix in the grid cell itself).
    if (columnType === InsightsColumnType.Json || typeof value === 'object') {
