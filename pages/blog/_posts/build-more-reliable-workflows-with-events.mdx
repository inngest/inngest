...
createStepFunction("Contacts Import and enrichment", "api/contact_list.uploaded",
  ({ event, tools }) => {
    const { isValid, errors } = tools.run("Validate upload contents", async () => {
    ...
    if (listReviewedEvent.data.is_approved === false) {
      return tools.run("Delete uploaded contact lists", () => { /* ...*/ });
    }

    const { totalUsersAdded } = tools.run("Create contacts in CRM", async () => {
    ...
    tools.run("Notify user of successful import", async () =>
...
All code within each `tools.run()` callback will be run independently and will be retried upon failure, resuming from where the function left off. Using `tools.waitForEvent()` enable the workflow to wait for additional user interactions or events via [event coordination](/patterns/event-coordination-for-lost-customers?ref=blog-build-more-reliable-workflows-with-events). More info on these and other `tools` are [in our docs](/docs/functions/multi-step?ref=blog-build-more-reliable-workflows-with-events#tools).
