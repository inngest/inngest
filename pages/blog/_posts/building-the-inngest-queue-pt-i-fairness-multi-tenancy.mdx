...
Attempting to build a system like this with any off-the-shelf queuing system is going to quick struggle. Here are some of the key challenges that you will encounter:

- **Fairness**.  You sends 5,000 events per second. Not hard to manage, but in a single queue your jobs will block other customers work from running. That's not fair. One user should not be able to block another's work.
