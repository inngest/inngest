...
2. Set the `InngestClient`'s `is_production` constructor argument to `false`.
...
`InngestClient`'s `is_production` constructor argument is useful for disabling production mode based on whatever logic you want. For example, you could control it using the `FLASK_ENV` environment variable:
```py
import inngest
