...
By default, each function is retried 4 times using [backoff](https://github.com/inngest/inngest/blob/main/pkg/backoff/backoff.go#L10-L22) with jitter, for a total
of 5 attempts per step.
...
* **Error** - Any error was thrown indicating a potentially temporary failure (think: `500`). **This will be retried according to the retry policy (4 times, by default)**.
