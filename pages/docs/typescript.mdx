...
export default inngest.createStepFunction(
  ...
  tools.run("Send welcome email", async () => {
    // "event" will be fully typed provide typesafety within this function
    return await email.send("welcome", event.data.email);
  });

  // We wait up to 2 days for the user to set up their account
  const accountSetupCompleted = tools.waitForEvent("user/account.setup.completed", {
    timeout: "2d",
  ...
  if (!accountSetupCompleted) {
    tools.run("Send setup account guide", async () => {
...
// ...
const accountSetupCompleted = tools.waitForEvent<UserAccountSetupCompleted>("user/account.setup.completed", {
  timeout: "2d",
  match: "data.user_id"
});
```
