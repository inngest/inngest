...
In the above example, we fetch all users within a single step.  This step will retry on error.  Once that’s complete (via `await`), we create an *array of events* which are sent to Inngest via [`step.sendEvent()`](/docs/reference/functions/step-send-event).  Each of these events trigger new functions for specific users.
