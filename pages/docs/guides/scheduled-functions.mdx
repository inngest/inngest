...
You can create scheduled functions that run in any timezone using the SDK's `createScheduledFunction`:
...
export const prepareWeeklyDigest = inngest.createScheduledFunction(
  "Prepare weekly digest",
  "TZ=Europe/Paris 0 12 * * 5",
  async () => {
    // Load all the users from your database:
    const users = await db.load("SELECT * FROM users");
