...
By default, each step is retried 3 times using [exponential backoff](https://github.com/inngest/inngest-cli/blob/main/pkg/backoff/backoff.go) with jitter. Retries can be customized using the `status` or `statusCode` response:
