...
image: /assets/docs/os-arch.png
position: 1
---

Inngest is an **event-driven queue**.  Instead of configuring queues, enqueueing in your API, deploying workers, and manually monitoring the backlog, Inngest decouples your queue from your systems by using events. 
...
- **Functions**, which are triggered by events or on a schedule

Whenever something happens, you can send an event to Inngest.  You can then build functions which are automatically triggered by events.  This has a few benefits:

- Because your core services are only sending events, you don't need to re-deploy these when adding new workers.
- You can
