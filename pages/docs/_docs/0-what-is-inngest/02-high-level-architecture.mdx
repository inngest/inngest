...
- The queue can be any implementation.  We use an internal implementation, and we're open-sourcing implementations for Redis, Postgres, and currently existing systems such as Celery.  The backing implementation doesn't necessarily matter, as we're abstracting the queue via state independently.
...
Inngest is offered as a cloud, with an [open-source local implementation for development](https://github.com/inngest/inngest-cli).  We're also implementing a fully self-hostable version of the cloud, with multiple backing state stores (eg. Redis and Postgres).
