...
Functions are always *step functions*, even if you only want to run a single step.  This lets you grow into building complex multi-language workflows in the future.  For more information on building step functions read the step function documentation.

That’s it — your function will have been generated in the `./prisma-background-job` directory! Let’s dive into writing the code that connects to your database.
...
This generates a package we can import within `src/index.ts` . Let’s update the step’s code to use this package, find the user that made the payment, then save the payment to the database:
