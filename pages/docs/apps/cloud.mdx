...
<details>
  <summary><strong>Why do I have duplicated apps?</strong></summary>

- Each app name is considered a persistent identifier. [Since the name of the app is determined by the ID passed to the serve handler from the Inngest client](/docs/apps#apps-in-sdk), changing that ID will result in Inngest not recognizing the app name during the next sync. As a result, Inngest will create a new app. 
