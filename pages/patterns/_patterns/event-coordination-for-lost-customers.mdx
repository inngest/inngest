...
export default inngest.createFunction<Added>(
...
We only want to run this function once for each cart_id.  A user might add 3 products, but they should only receive a single reminder email.  In this case, we set a `cancellation` option when defining the function which cancels any running or paused functions.  Any time another product is added to the same cart ID, older runs of this function are cancelled.  For example:
...
- TODO: Link to SDK `step.waitForEvent` docs
- TODO: Link to SDK `step.run` docs
- TODO: Link to SDK `step.sleep` docs for alternative approaches
- TODO: Link to auto-cancellation
*/}
