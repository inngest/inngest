...
2. Create an HTTP handler for a webhook endpoint that verifies [the webhook payload signature](https://stripe.com/docs/webhooks/signatures) and pushes webhook events to a particular queue
...
* Create a [webhook "source" in the Inngest dashboard](https://app.inngest.com/sources/new#Webhook) which will give you a unique URL which you can add to any third party service that
* Connect a [built-in integration via the Inngest dashboard](https://app.inngest.com/sources/new) which support event sources.
...
createFunction("Downgrade customer", "stripe/invoice.payment_failed", async ({ event }) => {
