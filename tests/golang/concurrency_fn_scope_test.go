...
func TestConcurrency_ScopeFunction(t *testing.T) {
	...
	for i := 0; i < (numEvents*(fnDuration+requeueDelay))+5; i++ {
...
func TestConcurrency_ScopeFunction_FanOut(t *testing.T) {
	...
	// Eventually, within a generous timeout after the total function duration,
	// all events have been processed by both functions.
	require.Eventually(t, func() bool {
