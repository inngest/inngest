...
Schedule another Inngest function ("deferred run") from inside the current run. Reported by the SDK as `OpcodeDeferAdd` / `OpcodeDeferCancel`. Deferred runs are enqueued at parent-run finalize via `inngest/deferred.schedule` events.
...
- [`pkg/execution/state/redis_state/lua/setDeferStatus.lua`](../pkg/execution/state/redis_state/lua/setDeferStatus.lua) — cancel transition; releases input bytes from the aggregate budget
...
- [`pkg/execution/executor/executor.go`](../pkg/execution/executor/executor.go) — `handleGeneratorDeferAdd`, `handleGeneratorDeferCancel`
...
- `enums.DeferStatus` — `AfterRun` (will schedule), `Aborted` (user-cancelled), `Rejected` (system-rejected). Aborted and Rejected are terminal.
...
Defers are reported as "lazy ops" (`OpcodeDeferAdd`, `OpcodeDeferCancel`); identified by `enums.OpcodeIsLazy`. Differ from normal step ops in five ways:
...
- **Processed inline.** No queue item, no follow-up SDK request. The executor handles them synchronously in `handleGeneratorDeferAdd` / `handleGeneratorDeferCancel` and they're done as soon as those return.
...
3. Meta entry stays so retransmits hit the terminal-sticky check in `saveDefer.lua` and no-op.
