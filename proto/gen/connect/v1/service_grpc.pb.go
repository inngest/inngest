...
func RegisterConnectGatewayServer(s grpc.ServiceRegistrar, srv ConnectGatewayServer) {
	// If the following call panics, it indicates UnimplementedConnectGatewayServer was
...
func RegisterConnectExecutorServer(s grpc.ServiceRegistrar, srv ConnectExecutorServer) {
	// If the following call panics, it indicates UnimplementedConnectExecutorServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
