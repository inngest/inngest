...
type quackAppendInserter struct{ appender *driver.QuackAppender }
...
func (a appenderInserter) insertBatch(ctx context.Context, rows []spanRow) error {
	...
	appender, err := duckdbgo.NewAppender(a.conn, driver.DuckLakeAlias, "main", "run_trace_spans")
	if err != nil {
...
func duckLakeOptions(tb testing.TB, rowLimit int) *driver.DuckLakeOptions {
	dir := tb.TempDir()
...
func openJSONLines(rowLimit int) func(testing.TB) spanInserter {
	return func(tb testing.TB) spanInserter {
		...
		if err := duckdb.Migrate(tb.Context(), db, true); err != nil {
...
var quackAppendColumns = []driver.QuackColumnKind{
...
func openQuackAppend(rowLimit int) func(testing.TB) spanInserter {
	return func(tb testing.TB) spanInserter {
		...
		appender, err := driver.NewQuackAppender(tb.Context(), db, driver.DuckLakeAlias, "main", "run_trace_spans", quackAppendColumns)
		if err != nil {
...
// doc comment).
func duckLakeAttachStmts(opts *driver.DuckLakeOptions) []string {
	rowLimit := opts.DataInliningRowLimit
	switch {
	case rowLimit == 0:
		rowLimit = driver.DefaultDataInliningRowLimit
	case rowLimit < 0:
		rowLimit = 0
	}
	return []string{
		...
		fmt.Sprintf(
			...
			opts.CatalogPath, driver.DuckLakeAlias, strings.TrimSuffix(opts.DataPath, "/"), rowLimit,
		),
	}
}
...
// pkg/db/driver.DuckLakeOptions.DataInliningRowLimit's doc comment) — so the
...
func TestAppenderStoresIdenticalRowToSQLInsert(t *testing.T) {
	db, connector := openEmbeddedConnector(t, driver.DefaultDataInliningRowLimit)
	...
	fetch := func(runID string) (name string, attributes any) {
		row := db.QueryRowContext(t.Context(),
			"SELECT name, attributes FROM "+driver.DuckLakeAlias+".run_trace_spans WHERE run_id = ?;", runID)
