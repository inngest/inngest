...
func main() {
	...
	sampleLimit := flag.Int("sample-limit", 200, "max rows to sample per table from -source as generation templates")
...
func run(ctx context.Context, cfg runConfig) (Summary, error) {
	...
	cfg.logf("sampling up to %d row(s) per table from source", cfg.SampleLimit)
	...
	if sampled {
		cfg.logf("sampled %d tenant(s), %d span name(s), %d event name(s) from source", len(tmpl.Tenants), len(tmpl.SpanNames), len(tmpl.EventNames))
	} else {
	...
	return GenerateAndInsert(ctx, targetConnector, rng, tmpl, genCfg, cfg.BatchSize, cfg.Parallelism, onProgress)
}
