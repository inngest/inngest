		...
		Traces: []TraceTemplate{
			{
				...
				EndedOffset:   new(600 * time.Millisecond),
			},
		},
		...
		MetadataProfiles: []MetadataProfile{
			{
				Items: []MetadataTemplateItem{
					...
					{SpanID: "seeded-step-1", Scope: "step", StepID: new("seeded-step-1"), StepIndex: new(0), StepAttempt: new(1), Kind: "user.custom", IsUser: true, Values: `{"seeded":true}`},
				},
			},
		},
	}
...
func sampleRuns(ctx context.Context, db *sql.DB, limit int) (tenants []Tenant, statuses, inputs, outputs []string, err error) {
	...
	;`, driver.DuckLakeAlias, driver.DuckLakeAlias)
...
func sampleTraces(ctx context.Context, db *sql.DB, limit int) ([]TraceTemplate, error) {
	...
	JOIN run_timing rt ON rt.run_id = s.run_id;`, driver.DuckLakeAlias, driver.DuckLakeAlias, driver.DuckLakeAlias)
	rows, err := db.QueryContext(ctx, query, limit)
	if err != nil {
	...
	for rows.Next() {
		...
		if !ok {
			...
			if rawStartedAt != nil {
				...
				b.startedOffset = new(startedAt.Sub(queuedAt))
			}
			if rawEndedAt != nil {
				...
				b.endedOffset = new(endedAt.Sub(queuedAt))
			}
			byRun[runID] = b
...
func sampleEventProfiles(ctx context.Context, db *sql.DB, limit int) ([][]EventTemplate, error) {
	...
	FROM run_events re
	JOIN %s.events e ON e.internal_id = re.event_internal_id;`, driver.DuckLakeAlias, driver.DuckLakeAlias, driver.DuckLakeAlias)
	rows, err := db.QueryContext(ctx, query, limit)
	if err != nil {
...
func sampleMetadata(ctx context.Context, db *sql.DB, limit int) ([]MetadataProfile, error) {
	...
	JOIN run_timing rt ON rt.run_id = m.run_id;`, driver.DuckLakeAlias, driver.DuckLakeAlias, driver.DuckLakeAlias)
	rows, err := db.QueryContext(ctx, query, limit)
	if err != nil {
