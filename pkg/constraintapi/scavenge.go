...
func (r *redisCapacityManager) Scavenge(ctx context.Context, options ...scavengerOpt) (*ScavengeResult, errs.InternalError) {
	...
	res, err := r.scavengeShard(ctx, r.client, now, o)
	if err != nil {
		return nil, errs.Wrap(0, false, "failed to scavenge: %w", err)
	}

	// Report metrics per queue shard
	res.Report(ctx, r.shardName)

	return res, nil
}

func (r *redisCapacityManager) scavengeShard(ctx context.Context, client rueidis.Client, now time.Time, o *scavengerOptions) (*ScavengeResult, error) {
	start := time.Now()
	defer func() {
	...
	// Peek accounts
	res, err := r.peekScavengerShard(ctx, client, o.accountsPeekSize, now)
	if err != nil {
