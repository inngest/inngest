...
var Attrs = struct {
	...
	// StepParentIDs are the steps this one waited for: the join it sits after,
	// or the single step it follows in a chain. Reported by the SDK, which can
	// see the promise combinator being built; never inferred here.
	StepParentIDs attr[*[]string]

	// StepParentAlternateIDs are steps that could have unblocked this one but
	// did not — the losing side of a race. Ordering, not dependency.
	StepParentAlternateIDs attr[*[]string]
	...
	StepParentIDs:                      StringSliceAttr("step.parent_ids"),
	StepParentAlternateIDs:             StringSliceAttr("step.parent_alternate_ids"),
