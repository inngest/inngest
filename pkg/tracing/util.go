...
// ExecutionDriverResponseAttrs applies details from the given `DriverResponse`
// to the executor.execution span after execution completes. It does not attach
// step.output; use finalizeDriverResponseAttrs in finalize.go for the run span.
func ExecutionDriverResponseAttrs(resp *state.DriverResponse) *meta.SerializableAttrs {
	rawAttrs := meta.NewAttrSet()
