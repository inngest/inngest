...
func TestSetupDualWriteReturnsNilListenerWhenBinaryMissing(t *testing.T) {
	l, db := setupDualWrite(context.Background(), true, "/nonexistent/duckdb", t.TempDir())
	require.Nil(t, l)
	require.Nil(t, db)
}
...
func TestSetupDualWriteReturnsListenerWhenBinaryPresent(t *testing.T) {
	...
	l, _ := setupDualWrite(context.Background(), true, binPath, t.TempDir())
	require.NotNil(t, l)
...
func TestSetupDualWriteNeverBlocksOnFailure(t *testing.T) {
	...
	go func() {
		l, _ := setupDualWrite(context.Background(), true, "/nonexistent/duckdb", t.TempDir())
		done <- l
	}()
...
func TestSetupDualWriteIsOptInAndOffByDefault(t *testing.T) {
	...
	l, db := setupDualWrite(context.Background(), false, binPath, stateDir)
	require.Nil(t, l)
	require.Nil(t, db)
...
func TestSetupDualWriteResolvesRelativeStateDir(t *testing.T) {
	...
	l, _ := setupDualWrite(context.Background(), true, binPath, "relative-state")
	require.NotNil(t, l)
...
func TestSetupDualWriteReturnsDBHandleOnSuccess(t *testing.T) {
	...
	l, db := setupDualWrite(context.Background(), true, binPath, t.TempDir())
	require.NotNil(t, l)
	require.NotNil(t, db)
	t.Cleanup(func() { stopDualWrite(context.Background(), l) })
}
...
func TestDualWriteEndToEndDoesNotAffectPrimaryPath(t *testing.T) {
	...
	l, _ := setupDualWrite(ctx, true, binPath, stateDir)
	require.NotNil(t, l)
...
func TestSetupDualWriteTwoInstancesDoNotCollideOnQuackPort(t *testing.T) {
	...
	l1, _ := setupDualWrite(ctx, true, binPath, t.TempDir())
	...
	l2, _ := setupDualWrite(ctx, true, binPath, t.TempDir())
