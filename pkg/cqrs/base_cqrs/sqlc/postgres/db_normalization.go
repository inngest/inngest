...
func (q NormalizedQueries) GetSpansByRunIDsAndName(ctx context.Context, arg sqlc_sqlite.GetSpansByRunIDsAndNameParams) ([]*sqlc_sqlite.GetSpansByRunIDsAndNameRow, error) {
	rows, err := q.db.GetSpansByRunIDsAndName(ctx, GetSpansByRunIDsAndNameParams{
		RunIds: arg.RunIds,
		Name:   arg.Name,
	})
	if err != nil {
	...
	sqliteRows := make([]*sqlc_sqlite.GetSpansByRunIDsAndNameRow, len(rows))
	for i, row := range rows {
		sqliteRows[i], _ = row.ToSQLite()
	}
