...
func TestGetTraceRunsFiltersByIsDeferred(t *testing.T) {
	...
	deferredOnly.IsDeferred = new(true)
	...
	notDeferredOnly.IsDeferred = new(false)
...
func TestGetTraceRunsAppliesOutputCELFilter(t *testing.T) {
	...
	// inngest.runs.output holds whatever DriverResponse.GetTraceFunctionOutput
	// produced for the run's own span (see pkg/execution/dualwrite/tracing.go's
	// materializeRuns, which copies run_trace_spans.output straight across) —
	// that always wraps trace data as {"data":T} or {"error":T} ("NOTE: This
	// always returns a wrapped response... We ALWAYS wrap trace data.", see
	// GetTraceFunctionOutput's own doc comment), so output.* targets the
	// "data" key, matching run.SpanEventDuckDBConverter and the SQLite/
	// Postgres CEL dialects' own spans.output.data convention.
