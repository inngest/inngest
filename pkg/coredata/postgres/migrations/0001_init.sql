...
CREATE TABLE public.action_versions (
...
ALTER TABLE ONLY public.function_versions
  ADD CONSTRAINT function_versions_function_id FOREIGN KEY (function_id) REFERENCES public.functions(function_id) ON DELETE CASCADE;

CREATE INDEX function_versions_valid ON public.function_versions USING btree (valid_from, valid_to, function_id);
...
ADD CONSTRAINT function_triggers_function_id_version FOREIGN KEY (function_id, version) REFERENCES public.function_versions(function_id, version) ON DELETE CASCADE;

-- indexes to match by trigger
CREATE INDEX function_triggers_event_name_function_id ON public.function_triggers USING btree (event_name, function_id) WHERE event_name IS NOT NULL;
CREATE INDEX function_triggers_schedule_function_id ON public.function_triggers USING btree (schedule, function_id) WHERE schedule IS NOT NULL;


-- +goose Down
DROP TABLE public.action_versions;
DROP TABLE public.function_triggers;
DROP TABLE public.function_versions;
