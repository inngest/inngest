...
func (a router) getSpanOutput(w http.ResponseWriter, r *http.Request) {
	...
	// The preview span identifier (created by encodeSpanOutputID) does not
	// embed account/workspace IDs, so they will be zero-value UUIDs.
	// When the identifier carries auth info, validate it matches the caller.
	// In all cases, stamp the caller's auth into the identifier so the
	// downstream reader (especially the cloud RPC) receives proper
	// workspace-scoped context.
	zeroUUID := uuid.UUID{}
	if id.AccountID != zeroUUID || id.WorkspaceID != zeroUUID {
		if id.AccountID != auth.AccountID() || id.WorkspaceID != auth.WorkspaceID() {
			_ = publicerr.WriteHTTP(w, publicerr.Errorf(404, "Span output not found"))
			return
		}
	}
	id.AccountID = auth.AccountID()
	id.WorkspaceID = auth.WorkspaceID()
	...
	if spanData.IsError {
		...
		if stepErr.Message == "" && stepErr.Stack == "" {
			// Only overwrite stack if we have neither message nor stack
			stepErr.Stack = string(spanData.Data)
		}
		resp.Error = &stepErr
	} else {
