...
// parseRunFilters parses shared status, time range, function_id, and app_id filters from the request.
func parseRunFilters(r *http.Request, filter *cqrs.GetTraceRunFilter) error {
	...
	for _, id := range r.URL.Query()["function_id"] {
		...
		if err != nil {
			return publicerr.Wrapf(err, 400, "Invalid function_id: %s", id)
		}
	...
	for _, id := range r.URL.Query()["app_id"] {
		parsed, err := uuid.Parse(id)
		if err != nil {
			return publicerr.Wrapf(err, 400, "Invalid app_id: %s", id)
		}
...
func (a router) listRuns(w http.ResponseWriter, r *http.Request) {
	...
	if a.opts.FunctionReader == nil {
		_ = publicerr.WriteHTTP(w, publicerr.Errorf(501, "Run listing not available"))
		return
	}
	...
	if err := parseRunFilters(r, &filter); err != nil {
	...
	opts := cqrs.GetTraceRunOpt{
		...
		Items:  uint(limit + 1), // Fetch one extra to determine hasMore
	}
	...
	// Determine hasMore from the extra item, then trim before enrichment
