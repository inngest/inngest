...
type checkpointAsyncSteps struct {
	...
	// RequestID is the ULID the SDK received in its dispatch payload. It is
	// logged and forwarded for tracing.
	RequestID string `json:"request_id"`
	// GenerationID is the dispatch generation. The API rejects mismatches against
	// the queue item's current dispatch as stale (HTTP 409).
