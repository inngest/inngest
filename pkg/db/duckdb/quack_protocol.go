...
// ("" when untagged — e.g. "JSON" for DuckDB's JSON type), and, depending on
// id, either LIST's single unnamed element type (child, nested at type_info's
// field200) or STRUCT's named child fields (structFields, a counted list at
// that same field200 — see decodeQuackLogicalType's doc comment for why the
// two shapes differ despite sharing a field number).
...
// object — {100: extraTypeInfoKind byte, 101: alias string, 200: a nested
// child LogicalType for LIST, or a STRUCT's child fields, optionally more
// structured fields for types like DECIMAL/ENUM this client doesn't
// understand}. Only this shape (verified empirically for DuckDB's JSON, LIST,
// and STRUCT types) is decoded; anything with additional fields beyond that —
// surfaced as an unexpected field id where the terminator was expected —
// still errors, exactly as before, since this client has no way to interpret
// those types' extra structure.
//
// field200 carries two different shapes depending on id, despite sharing a
// field number: for LIST it's a single nested LogicalType object with no
// count prefix (decoded directly, recursively); for STRUCT it's a raw
// ULEB128 count followed by that many child entries, each an object with
...
// Confirmed against a real duckdb subprocess (SELECT {'a': 1, 'b': 'x'} AS s)
// by cross-referencing decoded field names/types/terminator counts against
// the exact byte offsets of known landmarks before and after the LogicalType
// object (result_names' field2 marker, and the DataChunk columns' field102
// marker for the matching per-chunk type).
