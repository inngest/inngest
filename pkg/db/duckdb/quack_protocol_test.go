...
// confirmed against a real duckdb subprocess (DECIMAL(10,2)): id=21,
// type_info={100: extraTypeInfoKind byte (value 2 observed for
...
func writeDecimalLogicalType(w *quackWriter, width, scale byte) {
	...
	w.beginObject()
	w.writeByte(100, 2) // extraTypeInfoKind — value observed empirically for DecimalTypeInfo
	w.writeByte(200, width)
	w.writeByte(201, scale)
...
// writeEnumLogicalType writes an ENUM LogicalType object — the shape
// confirmed against a real duckdb subprocess (ENUM('a','b')): id=104,
// type_info={100: extraTypeInfoKind byte (value 6 observed for
// EnumTypeInfo), 200: the dictionary's internal physical-size marker as a
// plain ULEB128 scalar (unused by this client beyond staying positioned on
...
func writeEnumLogicalType(w *quackWriter, values []string) {
	...
	w.buf.WriteByte(1)
	w.beginObject()
	w.writeByte(100, 6) // extraTypeInfoKind — value observed empirically for EnumTypeInfo
	w.writeByte(200, 2) // internal dictionary physical-size marker — unused by this client
	w.writeFieldID(201)
	w.beginList(uint64(len(values)))
...
// TestQuackLogicalTypeName pins typeName()'s output for every LogicalTypeId
// this client recognizes to the exact string DuckDB's own DESCRIBE prints
// for that type (confirmed against a real duckdb subprocess) — this is what
// lets DuckDBToColumnType (pkg/duckdb/insights/columntype.go) bucket a
// quack-sourced type identically to a DESCRIBE-sourced one.
func TestQuackLogicalTypeName(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			require.Equal(t, tc.want, tc.lt.typeName())
		})
	}
}
...
func writeStructLogicalType(w *quackWriter, names []string, typeWriters []func()) {
	...
	w.buf.WriteByte(1) // type_info nullable "present" marker
	w.beginObject()
	w.writeByte(100, 5) // extraTypeInfoKind — value observed empirically, not otherwise interpreted
	w.writeFieldID(200)
	w.beginList(uint64(len(names)))
	for i, name := range names {
		w.writeString(0, name)
		w.writeFieldID(1)
		typeWriters[i]()
		w.endObject() // entry terminator
	}
	w.endObject() // type_info terminator
	w.endObject() // outer LogicalType terminator
}
...
func TestQuackDecodeChunkStructDecodesRowToMap(t *testing.T) {
	...
	values, err := c.columns[0].values()
	require.NoError(t, err)
	require.Equal(t, []any{map[string]any{"a": int64(1), "b": "x"}}, values)
}

// buildStructChunkTwoRowsSecondNull builds a two-row STRUCT{a: INTEGER}
// DataChunk with the second row's whole struct value marked SQL NULL at the
// vector level (field101 validity mask), mirroring
// buildFlatIntegerChunkWithValidity's convention: the child column still
// carries a (garbage-but-harmless) entry for the NULL row, exactly like every
...
func buildStructChunkTwoRowsSecondNull(t *testing.T) []byte {
	...
	w.beginObject()
	w.writeBool(100, true) // hasValidity: row1 is NULL
	w.writeFieldID(101)
	w.writeData([]byte{0b01}) // row0 valid, row1 NULL
	w.writeFieldID(103)
	w.beginList(1) // 1 child
	...
	data := make([]byte, 8)
	data[0] = 7 // row0 = 7
	data[4] = 0 // row1 placeholder, unused since parent validity marks it NULL
	w.writeData(data)
	w.endObject()
...
func TestQuackDecodeChunkStructNullRowDecodesNil(t *testing.T) {
	...
	values, err := c.columns[0].values()
	require.NoError(t, err)
	require.Equal(t, []any{map[string]any{"a": int64(7)}, nil}, values)
}

// buildStructChunkWithNullField builds a one-row STRUCT{a: INTEGER, b:
// VARCHAR} DataChunk where the struct itself is non-NULL but its "b" child
// field is SQL NULL — a child column's own validity mask, independent of the
...
func buildStructChunkWithNullField(t *testing.T) []byte {
	...
	w.beginObject()
	w.writeBool(100, false) // struct itself has no NULL rows
	w.writeFieldID(103)
	w.beginList(2)
	...
	w.writeBool(100, false)
	w.writeFieldID(102)
	w.writeData([]byte{9, 0, 0, 0})
	w.endObject()

	// child1 (b: VARCHAR), NULL
	w.beginObject()
	w.writeBool(100, true)
	w.writeFieldID(101)
...
func TestQuackDecodeChunkStructWithNullFieldDecodesNilForThatField(t *testing.T) {
	...
	values, err := c.columns[0].values()
	require.NoError(t, err)
	require.Equal(t, []any{map[string]any{"a": int64(9), "b": nil}}, values)
}
...
func buildListOfStructChunk(t *testing.T) []byte {
	...
	w.beginObject()
	w.writeBool(100, false) // no NULL lists
	w.writeUint64(104, 2)   // flattened child element count: 2 structs
	w.writeFieldID(105)
	w.beginList(1) // 1 list_entry_t row
	w.beginObject()
	w.writeUint64(100, 0) // offset
	w.writeUint64(101, 2) // length
	w.endObject()

	w.writeFieldID(106)
	w.beginObject() // child vector: STRUCT{a: INTEGER}, flattened to 2 rows
	w.writeBool(100, false)
	w.writeFieldID(103)
	w.beginList(1) // 1 child field

	w.beginObject() // "a" child column, 2 flattened rows: 1, 2
	w.writeBool(100, false)
	w.writeFieldID(102)
