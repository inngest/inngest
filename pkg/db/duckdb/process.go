...
func (p *process) bootstrapStateDirLocked(ctx context.Context) error {
	...
	stmts := []string{
		...
		fmt.Sprintf("SET extension_directory=%s;", extLiteral),
	}
...
func (c *Connector) Connect(ctx context.Context) (driver.Conn, error) {
	if c.proc == nil {
		...
		p, err := startProcessWithDuckLake(ctx, binPath, c.opts.DBFile, c.opts.DuckLake, c.opts.QuackAddr, withStateDir(c.opts.StateDir))
		if err != nil {
			return nil, err
		}
		c.proc = p
