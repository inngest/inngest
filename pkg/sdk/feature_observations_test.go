...
func TestFeatureObservationsJSON(t *testing.T) {
	...
	r.Equal(int(sdkfeatureobs.AIMetadataExtractionReadinessReason_AI_METADATA_EXTRACTION_READINESS_REASON_OTEL_PROVIDER_MISSING), int(decoded[0].GetAiMetadataExtraction().GetReadinessReason()))
	r.Equal(int(sdkfeatureobs.ExtendedTracesBehavior_EXTENDED_TRACES_BEHAVIOR_AUTO), int(decoded[1].GetExtendedTraces().GetConfig().GetBehavior()))
}
...
func TestFeatureObservationsJSONAcceptsEnumNames(t *testing.T) {
	...
	require.Equal(
		...
		observations[0].GetSendEvents().GetReadinessReason(),
	)
	require.True(t, observations[0].GetSendEvents().GetConfig().GetHasEventKey())
	require.True(t, observations[0].GetSendEvents().GetConfig().GetHasEventApiOriginOverride())
}
