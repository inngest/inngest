...
func (d *debugAPI) GetShadowPartition(ctx context.Context, req *pb.ShadowPartitionRequest) (*pb.ShadowPartitionResponse, error) {
	...
	pt, err := d.partReader.PartitionByID(ctx, shard, scope, req.GetPartitionId())
	if err != nil {
...
func (d *debugAPI) GetBacklogs(ctx context.Context, req *pb.BacklogsRequest) (*pb.BacklogsResponse, error) {
	...
	backlogReader, ok := shard.(queue.BacklogOperations)
	if !ok {
		return nil, fmt.Errorf("queue shard %q does not support backlog reads", shard.Name())
	}
	iter, err := backlogReader.BacklogsByPartition(ctx, req.GetPartitionId(), time.Time{}, until)
	if err != nil {
	...
	for bl := range iter {
		...
		size, _ := d.backlogReader.BacklogSize(ctx, shard, bl.BacklogID)
...
func (d *debugAPI) GetBacklogSize(ctx context.Context, req *pb.BacklogSizeRequest) (*pb.BacklogSizeResponse, error) {
	...
	size, err := d.backlogReader.BacklogSize(ctx, shard, req.GetBacklogId())
	if err != nil {
	...
	bl, err := d.backlogReader.BacklogByID(ctx, shard, req.GetBacklogId())
	if err == nil {
