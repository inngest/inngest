...
// ConsumePause consumes a pause, writing the consumed event data to run state.
//
// NOTE: This is the only PauseStore method that uses the sharded client. It mutates
// run state (not pauses) by storing the consumed data in the function run's action
// responses. The pause itself is deleted separately by pauses.Manager.
//
// The returned cleanup function is always nil - pauses.Manager provides the actual
// cleanup logic.
func (s *PauseStore) ConsumePause(ctx context.Context, p state.Pause, opts state.ConsumePauseOpts) (state.ConsumePauseResult, func() error, error) {
	...
	fnRunState := s.sharded.FunctionRunState()
