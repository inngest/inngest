...
func TestGlobalPartitions(t *testing.T) {
	r, rc := initRedis(t)
	defer rc.Close()
	...
	testcases := []struct {
		...
		},
		{
			...
			until:         clock.Now().Add(7 * time.Second).Truncate(time.Second),
			interval:      time.Second,
			expectedItems: 8, // the interval is inclusive so from 0,7 -> 8 items
		},
		{
			name:          "with batch size",
			num:           500,
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			items, err := q.GlobalPartitions(ctx, defaultShard, tc.from, tc.until,
				WithQueueItemIterBatchSize(tc.batchSize),
			)
			require.NoError(t, err)
			...
			var count int
			for part := range items {
				if _, seen := uniqueParts[part.ID]; seen {
					require.Fail(t, "duplicate partition", part)
				}
				uniqueParts[part.ID] = struct{}{}
				count++
			}
...
func TestGlobalShadowPartitions(t *testing.T) {
	r, rc := initRedis(t)
	defer rc.Close()
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			items, err := q.GlobalShadowPartitions(ctx, defaultShard, tc.from, tc.until,
				WithQueueItemIterBatchSize(tc.batchSize),
			)
			require.NoError(t, err)
			...
			var count int
			for part := range items {
				if _, seen := uniqueParts[part.PartitionID]; seen {
					require.Fail(t, "duplicate partition", part)
				}
				uniqueParts[part.PartitionID] = struct{}{}
				count++
			}
