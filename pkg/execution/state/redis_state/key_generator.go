...
type BatchKeyGenerator interface {
	...
	// BatchIdempotencetKey returns the key used to store the specific batch of
	// events, that is used to check if a batch event has already been appended to a batch.
	BatchIdempotencetKey(ctx context.Context, functionId uuid.UUID, batchId ulid.ULID) string
...
func (u batchKeyGenerator) BatchIdempotencetKey(ctx context.Context, functionId uuid.UUID, batchID ulid.ULID) string {
