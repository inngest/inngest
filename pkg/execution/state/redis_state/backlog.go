...
func (q *queue) BacklogFinishNormalize(ctx context.Context, b *QueueBacklog, sp *QueueShadowPartition) error {
	ctx = redis_telemetry.WithScope(redis_telemetry.WithOpName(ctx, "BacklogFinishNormalize"), redis_telemetry.ScopeQueue)

	shard := q.primaryQueueShard

	if shard.Kind != string(enums.QueueShardKindRedis) {
		return fmt.Errorf("unsupported queue shard kind for BacklogFinishNormalize: %s", shard.Kind)
	}
	kg := shard.RedisClient.kg
	...
	status, err := scripts["queue/backlogFinishNormalize"].Exec(
		redis_telemetry.WithScriptName(ctx, "backlogFinishNormalize"),
		shard.RedisClient.unshardedRc,
		keys,
		args,
	).ToInt64()
	if err != nil {
		return fmt.Errorf("error finishing backlog normalization: %w", err)
	}
	...
	case 1:
		return nil
	case -1:
		q.log.Warn("attempted to finish normalization but backlog is not empty", "backlog", b.BacklogID, "partition_id", sp.PartitionID)

		// Return without error to retry normalization
		return nil
	default:
		return fmt.Errorf("unknown status finishing backlog normalization: %v (%T)", status, status)
	}
}
