...
// produced for the given run/status, waiting for the listener's async batch
// flush to land it first.
func runRow(t *testing.T, db *sql.DB, runID string, status enums.StepStatus) map[string]any {
	t.Helper()
	require.Eventually(t, func() bool {
		var count int
		row := db.QueryRowContext(context.Background(),
			"SELECT count(*) FROM inngest.runs WHERE run_id = ? AND status = ?;", runID, status.String())
		_ = row.Scan(&count)
		return count == 1
	}, 2*time.Second, 20*time.Millisecond, "run row should land in inngest.runs after a batch flush")

	rows := selectRows(t, db,
		"SELECT run_id, function_id, status, event_ids, sessions FROM inngest.runs WHERE run_id = ? AND status = ?;",
		runID, status.String())
...
// TestListenerOnFunctionScheduledSetsEventIDsFromMetadataConfig proves the
// run.queued span's event.ids attribute (and so inngest.runs' event_ids
// column, via materializeRuns) sources from md.Config.EventIDs — the run's
// own persisted trigger event ID list (set once at Schedule time in
...
func TestListenerOnFunctionScheduledSetsEventIDsFromMetadataConfig(t *testing.T) {
	...
	row := runRow(t, db, md.ID.RunID.String(), enums.StepStatusQueued)
	require.Equal(t, []any{evt1.String(), evt2.String()}, row["event_ids"])
}
...
// never sets event_ids at all, rather than an empty array.
...
// normalizeRunSessions, which builds the same shape for the real run span's
// "event.sessions" attribute.
func TestListenerOnFunctionScheduledSetsSessionsFromTriggeringEvents(t *testing.T) {
	...
	row := runRow(t, db, md.ID.RunID.String(), enums.StepStatusQueued)
	require.Equal(t, []any{
		map[string]any{"key": "cart", "id": "123"},
		map[string]any{"key": "customer", "id": "acme"},
	}, row["sessions"])
}
...
// TestListenerOnStepRunFailedCreatesStepSpan proves OnStepRunFailed —
// executor.RunStepRunFailedLifecycle's counterpart to OnStepRunFinished for
// a step.run step that errors (will retry) or fails (final) — writes an
// executor.step span with the right status/attempt attributes and the same
// deterministic span identity executor.emitStepSpan computes for the real
// production span (RetryStepDynamicSeed for a retryable error, so it lands
// on its own row distinct from the step's eventual finalized span;
// FinalizedStepDynamicSeed for a final failure, the same identity
// OnStepRunFinished's success case would have used).
func TestListenerOnStepRunFailedCreatesStepSpan(t *testing.T) {
	tests := []struct {
		name       string
		status     enums.StepStatus
		attempt    int
		wantSpanID func(md sv2.Metadata, stepID string) string
	}{
		{
			...
			attempt: 2,
			wantSpanID: func(md sv2.Metadata, stepID string) string {
				return tracing.RetryStepSpanRefFromMetadataAndStepID(&md, stepID, 2).DynamicSpanID
			},
		},
		{
			name:    "final failure",
			...
			attempt: 3,
			wantSpanID: func(md sv2.Metadata, stepID string) string {
				return tracing.FinalizedStepSpanRefFromMetadataAndStepID(&md, stepID).DynamicSpanID
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			md := testMetadata(t)
			gen := statev1.GeneratorOpcode{ID: "step-1", Name: "my-step", Op: enums.OpcodeStepError}

			l.OnStepRunFailed(context.Background(), md, queue.Item{}, inngest.Edge{}, gen, tt.status, tt.attempt, time.Now())

			require.Eventually(t, func() bool {
				var count int
				row := db.QueryRowContext(context.Background(), "SELECT count(*) FROM inngest.run_trace_spans WHERE name = ?;", tracingv3.SpanNameStep)
				_ = row.Scan(&count)
				return count == 1
			}, 2*time.Second, 20*time.Millisecond, "executor.step span should land after a batch flush")

			rows := selectRows(t, db, "SELECT span_id, attributes FROM inngest.run_trace_spans WHERE name = ?;", tracingv3.SpanNameStep)
			require.Len(t, rows, 1)

			attrs, ok := rows[0]["attributes"].(map[string]any)
			require.True(t, ok, "attributes column should decode to a map, got %T", rows[0]["attributes"])
			require.Equal(t, tt.status.String(), attrs[meta.Attrs.DynamicStatus.Key()])
			require.EqualValues(t, tt.attempt, attrs[meta.Attrs.StepAttempt.Key()])

			require.Equal(t, tt.wantSpanID(md, gen.ID), rows[0]["span_id"])
		})
	}
}
...
func TestListenerOnDeferAddWritesDeferSpan(t *testing.T) {
	...
	// span_id is just whatever random id CreateDroppableSpan's normal ID
	// generator assigns — duckdbquery groups defer rows by the
	// defer.hashed_id attribute instead (see OnDeferAdd's own doc comment),
	// so nothing here depends on span_id's value beyond it existing.
	require.NotEmpty(t, rows[0]["span_id"])
}

// TestListenerOnDeferAbortUsesDistinctPhysicalSpanID proves OnDeferAbort's
// write lands under its own distinct physical span_id rather than reusing
// OnDeferAdd's — run_trace_spans requires span_id stay unique per row (see
// OnDeferAdd's own doc comment) — while duckdbquery's read side still
// collapses both rows into one logical defer via their shared
// defer.hashed_id attribute.
func TestListenerOnDeferAbortUsesDistinctPhysicalSpanID(t *testing.T) {
	db, cleanup := newTestDuckDB(t)
	defer cleanup()
	l := NewListener(db, func(o *setupOpts) { o.batchInterval = 20 * time.Millisecond })
	...
	l.OnDeferAbort(context.Background(), md, d.HashedID, d.FnSlug, "userland-2", time.Now())
	...
	require.NotEqual(t, rows[0]["span_id"], rows[1]["span_id"], "each physical row must have its own unique span_id")

	var sawAborted bool
	for _, row := range rows {
		attrs, ok := row["attributes"].(map[string]any)
		require.True(t, ok)
		require.Equal(t, d.HashedID, attrs[meta.Attrs.DeferHashedID.Key()], "both rows must share the same defer.hashed_id")
		if attrs[meta.Attrs.DeferStatus.Key()] == enums.DeferStatusAborted.String() {
			sawAborted = true
			// fn_slug/userland_id ride along on the abort op itself now —
			// see OnDeferAbort's own doc comment.
			require.Equal(t, d.FnSlug, attrs[meta.Attrs.DeferFnSlug.Key()])
			require.Equal(t, "userland-2", attrs[meta.Attrs.DeferUserlandID.Key()])
		}
	}
...
// via defer() gets is_deferred=true on its own inngest.runs row and its own
// DeferParentRunIDs/DeferParentFnSlug attrs — the two things
// GetTraceRunFilter.IsDeferred and GetRunDeferredFrom need to work on
// DuckDB. Unlike the parent-run side (see duckdbquery's
// TestGetRunDefersResolvesChildRunIDViaEventID), nothing here writes back
// onto the parent's own executor.defer span: OnDeferAdd's original write
// already carries the deterministic defer.event_id GetRunDefers resolves the
// child through.
func TestListenerOnFunctionScheduledLinksDeferredChild(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		var count int
		row := db.QueryRowContext(context.Background(),
			"SELECT count(*) FROM inngest.run_trace_spans WHERE name = 'executor.run.queued' AND run_id = ?;",
			childMD.ID.RunID.String())
		_ = row.Scan(&count)
		return count == 1
	}, 2*time.Second, 20*time.Millisecond, "child's own queued span should land after a batch flush")
...
func TestListenerOnFunctionScheduledOmitsIsDeferredForNormalRuns(t *testing.T) {
	...
	// 000001_baseline.sql) and the trigger derives it as
	// "defer_parent_fn_slug IS NOT NULL" -- a predicate that is itself never
	// NULL, so a normal run's row always carries a concrete false, not NULL.
	require.Equal(t, false, rows[0]["is_deferred"], "a normal (non-deferred) run must leave is_deferred false")
}
