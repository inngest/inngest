...
type SyncLifecycleListener interface {
	...
	// OnStepRunFailed is called synchronously when a plain step.run step
	// (enums.OpcodeStepError/OpcodeStepFailed) errors or fails — the
	// counterpart to OnStepRunFinished for the non-success outcomes it
	...
	// pkg/execution/defers.AbortFromOp). fnSlug/userlandID are the same
	// values the original OnDeferAdd call for this hashedID carried (sourced
	// from the abort op's own DeferAbortOpts.FnSlug/Userland.ID, which the
	// SDK sends on every abort) -- implementations that re-emit rather than
	// partially update a row (e.g. pkg/execution/dualwrite, whose flat
	// run_trace_spans model has no partial-update) can use them to keep that
	// row fully self-describing. now is the caller's own "this just
	// happened" timestamp -- see OnFunctionFinished.
	OnDeferAbort(ctx context.Context, md statev2.Metadata, hashedID, fnSlug, userlandID string, now time.Time)
...
func (NoopSyncLifecycleListener) OnStepRunFailed(context.Context, statev2.Metadata, queue.Item, inngest.Edge, statev1.GeneratorOpcode, enums.StepStatus, int, time.Time) {
}
...
func (NoopSyncLifecycleListener) OnDeferAbort(context.Context, statev2.Metadata, string, string, string, time.Time) {
}
