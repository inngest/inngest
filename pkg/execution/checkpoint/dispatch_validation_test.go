...
func TestDispatchValidation(t *testing.T) {
	t.Run("valid", func(t *testing.T) {
		for _, validationEnabled := range []bool{false, true} {
			t.Run(fmt.Sprintf("validation_enabled=%v", validationEnabled), func(t *testing.T) {
				...
				// checkpoint request.
				ac := newAsyncCheckpointForItem(qi)

				runState := newTestStateStore(t)
				md := state.Metadata{ID: ac.ID()}
				state.InitConfig(&md.Config)
				_, err := runState.Create(ctx, state.CreateState{Metadata: md})
				r.NoError(err)

				c, tracer := newCheckpointerWithState(t, q, runState, validationEnabled)
				tracer.On("CreateSpan", mock.Anything, mock.Anything, mock.Anything).
					Return(&meta.SpanReference{}, nil)

				r.NoError(c.CheckpointAsyncSteps(ctx, ac))

				// Ensure step processing resulted in a StateStore update.
				steps, err := runState.LoadSteps(ctx, ac.ID())
				r.NoError(err)
	...
	t.Run("invalid", func(t *testing.T) {
		...
		for _, validationEnabled := range []bool{false, true} {
			t.Run(fmt.Sprintf("validation_enabled=%v", validationEnabled), func(t *testing.T) {
				...
				tracer.On("CreateSpan", mock.Anything, mock.Anything, mock.Anything).
					Return(&meta.SpanReference{}, nil)
				err = c.CheckpointAsyncSteps(ctx, ac)
				if validationEnabled {
					r.ErrorIs(err, ErrStaleDispatch)
				} else {
					r.NoError(err)
				}
	...
	t.Run("missing request ID", func(t *testing.T) {
		...
		c, tracer := newCheckpointerWithState(t, q, runState, true)
		tracer.On("CreateSpan", mock.Anything, mock.Anything, mock.Anything).
			Return(&meta.SpanReference{}, nil)
	...
	t.Run("skip validation when checkpoint request was fast", func(t *testing.T) {
		...
		// started.
		ac.RequestStartedAt = time.Now().UnixMilli()
...
// newAsyncCheckpointForItem builds the AsyncCheckpoint that corresponds to a
// queue item enqueued by enqueueTestItem.
func newAsyncCheckpointForItem(qi queue.QueueItem) AsyncCheckpoint {
...
// newCheckpointerWithState wires up a Checkpointer against a real
// state.RunService so the test can read back the post-checkpoint store. Only
// the tracer is returned for expectation setup; the metrics provider is
// internal because the async path doesn't call into it.
func newCheckpointerWithState(
	t *testing.T,
	q queue.Queue,
	runState state.RunService,
	validationEnabled bool,
) (Checkpointer, *mockTracerProvider) {
	t.Helper()
