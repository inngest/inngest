...
// latencyQueueName returns a deterministic partition name for latency tracking.
func latencyQueueName(n int) string {
	return fmt.Sprintf("ffffffff-ffff-ffff-ffff-fffffffffff%x", n)
}
...
func (q *queueProcessor) enqueueLatencyJob(ctx context.Context, partition int) error {
	queueName := latencyQueueName(partition)
