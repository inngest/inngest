...
const latencyQueuePrefix = "ffffffff-ffff-ffff-ffff-fffffffffff"

// latencyQueueName returns a deterministic partition name for latency tracking.
func latencyQueueName(n int) string {
	return fmt.Sprintf("%s%x", latencyQueuePrefix, n)
}

// IsLatencyPartition reports whether the given ID matches a latency tracking
// partition name generated by latencyQueueName.
func IsLatencyPartition(id string) bool {
	return strings.HasPrefix(id, latencyQueuePrefix)
}
