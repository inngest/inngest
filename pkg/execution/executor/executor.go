...
func createCancellationPauses(ctx context.Context, pm pauses.Manager, l logger.Logger, now time.Time, idempontenceKey string, evtMap map[string]any, id sv2.ID, req execution.ScheduleRequest) error {
	for _, c := range req.Function.Cancel {
		...
		_, err := pm.Write(ctx, pauses.Index{WorkspaceID: req.WorkspaceID, EventName: c.Event}, &pause)
...
// createEagerCancellationForTimeout enqueues a system job in the future for eager cancellation of timed out jobs.
func createEagerCancellationForTimeout(ctx context.Context, producer queue.Producer, since time.Time, timeout *time.Duration, cancellationKind enums.CancellationKind, id state.Identifier) error {
	...
	err := producer.Enqueue(ctx, queue.Item{
...
func (e *executor) Execute(ctx context.Context, id state.Identifier, item queue.Item, edge inngest.Edge) (*state.DriverResponse, error) {
	...
	if md.Config.StartedAt.IsZero() {
		...
		if ef.Function.Timeouts != nil && ef.Function.Timeouts.Finish != nil {
			if err := createEagerCancellationForTimeout(ctx, e.queue, start, ef.Function.Timeouts.FinishDuration(), enums.CancellationKindFinishTimeout, id); err != nil {
				return nil, err
			}
		}
	}
