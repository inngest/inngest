	...
	case connectpb.GatewayMessageType_WORKER_REQUEST_EXTEND_LEASE:
		{
			...
			newLeaseID, err := c.svc.stateManager.ExtendRequestLease(ctx, c.conn.EnvID, c.conn.InstanceID, data.RequestId, leaseID, consts.ConnectWorkerRequestLeaseDuration)
			if err != nil {
				switch {
...
func (c *connectionHandler) establishConnection(ctx context.Context) (*state.Connection, *connecterrors.SocketError) {
	...
	if err := c.svc.stateManager.SetWorkerTotalCapacity(context.Background(), c.conn.EnvID, initialMessageData.InstanceId, initialMessageData.MaxConcurrentWorkerLeases); err != nil {
