...
// connection just long enough to extract its underlying quackSession, then
...
// quackSession and, if catalog is non-empty, switches that session's default
...
// cmd/duckdbseed/insert.go).
func resolveQuackSessionForAppender(ctx context.Context, driverConn any, catalog, errPrefix string) (*quackSession, error) {
