...
// which enforces DuckDB's own STANDARD_VECTOR_SIZE (2048) strictly.
// Confirmed against a real duckdb v2.1.0-alpha40409 subprocess via
// cmd/duckdbseed's actual seeding path (500 runs' worth of
// inngest.run_trace_spans rows, one flush): a single oversized chunk failed
// downstream expression evaluation with "Vector::SetSize out of range —
...
func TestQuackAppenderFlushesMoreThanOneVectorWorthOfRows(t *testing.T) {
	...
	_, err = db.ExecContext(t.Context(), "CREATE TABLE "+DuckLakeAlias+".many_rows_t "+
		"(id VARCHAR, attrs JSON);")
	require.NoError(t, err)

	appender, err := NewQuackAppender(t.Context(), db, DuckLakeAlias, "main", "many_rows_t",
	...
	row := db.QueryRowContext(t.Context(), "SELECT count(*) FROM "+DuckLakeAlias+".many_rows_t;")
	var gotCount int
	require.NoError(t, row.Scan(&gotCount))
	require.Equal(t, wantRows, gotCount)

	rows, err := db.QueryContext(t.Context(),
		"SELECT attrs FROM "+DuckLakeAlias+".many_rows_t WHERE id = 'row-"+fmt.Sprint(wantRows-1)+"';")
	require.NoError(t, err)
	defer rows.Close()
	require.True(t, rows.Next())
	got, err := ScanRowByName(rows)
	require.NoError(t, err)
	require.Equal(t, map[string]any{"i": float64(wantRows - 1)}, got["attrs"])
}
...
// TestQuackAppenderConcurrentPooledUseDoesNotSuperseded is
// TestQuackAppenderFromConnWritesConcurrentlyWithoutLoss's counterpart for
// the *pooled* constructor (NewQuackAppender, not NewQuackAppenderFromConn):
// concurrent workers each acquire their appender through the same *sql.DB
// pool rather than a manually-owned driver.Conn. Before NewQuackAppender
// held its acquired *sql.Conn open for the appender's own lifetime, this
// reproduced a real failure this package's own benchmark hit
// (cmd/duckdbbench's appender-based partition sweep): NewQuackAppender used
// to release its *sql.Conn back to the pool the moment it returned, so a
// second concurrent caller could be handed that exact connection while the
// first appender's stream was still open — its next PrepareRequest
// supersedes the first appender's in-flight one server-side
// ("duckdb: statement failed: superseded by a new query"), corrupting or
// losing that appender's batch. Every worker here uses the plain pooled
...
func TestQuackAppenderConcurrentPooledUseDoesNotSuperseded(t *testing.T) {
	...
	// chance to overlap with another worker's still-in-flight one — both
	// needed to make the race land reliably rather than depend on
	// incidental goroutine-scheduling luck.
	const quackConns = 4
	const workers = 20
	...
	_, err = db.ExecContext(t.Context(), "CREATE TABLE "+DuckLakeAlias+".concurrent_pooled_t (id VARCHAR);")
	require.NoError(t, err)
	...
	for w := range workers {
		...
		go func(worker int) {
			...
			for a := range appendersPerWorker {
				appender, err := NewQuackAppender(t.Context(), db, DuckLakeAlias, "main", "concurrent_pooled_t", []QuackColumnKind{QuackColumnVarchar})
				if err != nil {
					errs[worker] = err
					return
				}
				for i := range rowsPerAppender {
					if err := appender.AppendRow(fmt.Sprintf("worker-%d-appender-%d-row-%d-%s", worker, a, i, uuid.New())); err != nil {
						errs[worker] = err
						return
					}
				}
	...
	var count, distinct int
	require.NoError(t, db.QueryRowContext(t.Context(), "SELECT count(*) FROM "+DuckLakeAlias+".concurrent_pooled_t;").Scan(&count))
	require.NoError(t, db.QueryRowContext(t.Context(), "SELECT count(DISTINCT id) FROM "+DuckLakeAlias+".concurrent_pooled_t;").Scan(&distinct))
	require.Equal(t, wantRows, count)
	require.Equal(t, wantRows, distinct, "no row should have been lost or duplicated across concurrent pooled appenders")
}
