...
const (
	...
	// quackLogicalTypeVariant (types.hpp: LogicalTypeId::VARIANT) is
	...
	// are identical to an ordinary STRUCT's. See decodeQuackVariantVector's
	...
	// them.
	quackLogicalTypeVariant byte = 109
)
	...
	if ok, err := r.tryBeginProperty(101); err != nil {
		...
		if present {
			...
			if ok, err := r.tryBeginProperty(200); err != nil {
				...
				switch id {
				case quackLogicalTypeStruct, quackLogicalTypeUnion, quackLogicalTypeVariant:
					...
					// decodeQuackVariantVector). UNION's type_info is,
...
func decodeQuackVectorBody(r *quackReader, lt quackLogicalType, count int) (quackColumn, error) {
	...
	if lt.id == quackLogicalTypeVariant {
		return decodeQuackVariantVector(r, lt, count)
	}
...
// quackVariantType* values are VariantLogicalType's own ordinals
...
// overlapping names: e.g. quackLogicalTypeDate is 15, but a VARIANT-encoded
// DATE's own values[i].type_id is quackVariantTypeDate (19).
const (
	quackVariantTypeNull              byte = 0
	quackVariantTypeBoolTrue          byte = 1
	quackVariantTypeBoolFalse         byte = 2
	quackVariantTypeInt8              byte = 3
	quackVariantTypeInt16             byte = 4
	quackVariantTypeInt32             byte = 5
	quackVariantTypeInt64             byte = 6
	quackVariantTypeInt128            byte = 7
	quackVariantTypeUint8             byte = 8
	quackVariantTypeUint16            byte = 9
	quackVariantTypeUint32            byte = 10
	quackVariantTypeUint64            byte = 11
	quackVariantTypeUint128           byte = 12
	quackVariantTypeFloat             byte = 13
	quackVariantTypeDouble            byte = 14
	quackVariantTypeDecimal           byte = 15
	quackVariantTypeVarchar           byte = 16
	quackVariantTypeBlob              byte = 17
	quackVariantTypeUUID              byte = 18
	quackVariantTypeDate              byte = 19
	quackVariantTypeTimeMicros        byte = 20
	quackVariantTypeTimestampSec      byte = 22
	quackVariantTypeTimestampMillis   byte = 23
	quackVariantTypeTimestampMicros   byte = 24
	quackVariantTypeTimestampNanos    byte = 25
	quackVariantTypeTimeMicrosTZ      byte = 26
	quackVariantTypeTimestampMicrosTZ byte = 27
	quackVariantTypeInterval          byte = 28
	quackVariantTypeObject            byte = 29
	quackVariantTypeArray             byte = 30
	...
	// decodeQuackVariantValue's default case) rather than being silently
	// mis-decoded.
)

// decodeQuackVariantVector decodes a VARIANT column (LogicalTypeId::VARIANT,
// types.cpp). VARIANT is byte-for-byte a STRUCT at the wire/physical level
// (see quackLogicalTypeVariant's doc comment) with four fixed fields —
...
//
// This decodes the struct via the ordinary decodeQuackVector machinery
// (nothing above is VARIANT-specific at the wire level — see
// decodeQuackStructVector) and then, per row, unshreds values[0] — every
// row's value tree is rooted at index 0, per variant_visitor.hpp's
// ConvertVariantToValue/Visit — recursively via decodeQuackVariantValue,
// producing the actual Go value it represents: a map/slice/scalar, the same
// shape a JSON column already decodes to (see quackAliasJSON), rather than
// exposing this internal encoding to callers.
func decodeQuackVariantVector(r *quackReader, lt quackLogicalType, count int) (quackColumn, error) {
	structCol, err := decodeQuackStructVector(r, lt, count)
	if err != nil {
		return quackColumn{}, err
	}
	...
	for i, row := range rows {
		...
		if !ok {
			return quackColumn{}, fmt.Errorf("duckdb: quack VARIANT column decode: row %d is not a struct", i)
		}
		...
		v, verr := decodeQuackVariantValue(keys, children, values, data, 0)
		if verr != nil {
			return quackColumn{}, fmt.Errorf("duckdb: quack VARIANT column decode: row %d: %w", i, verr)
		}
		out[i] = v
	}
	return quackColumn{typeID: quackLogicalTypeVariant, precomputed: out, rowCount: count}, nil
}

// decodeQuackVariantValue recursively unshreds one VARIANT value, given its
// row's already-decoded keys/children/values/data fields (see
// decodeQuackVariantVector) and valueIndex: a row-local index into values —
...
// (quackHugeintBigInt, quackDecimalToString, quackDateToTime, ...).
func decodeQuackVariantValue(keys, children, values []any, data []byte, valueIndex int) (any, error) {
	...
	switch byte(typeID) {
	case quackVariantTypeNull:
		return nil, nil
	case quackVariantTypeBoolTrue:
		return true, nil
	case quackVariantTypeBoolFalse:
		return false, nil
	case quackVariantTypeInt8:
		return int64(int8(ptr[0])), nil
	case quackVariantTypeInt16:
		return int64(int16(le16(ptr))), nil
	case quackVariantTypeInt32:
		return int64(int32(le32(ptr))), nil
	case quackVariantTypeInt64:
		return int64(le64(ptr)), nil
	case quackVariantTypeInt128:
		return quackHugeintBigInt(ptr[:16], true).String(), nil
	case quackVariantTypeUint8:
		return int64(ptr[0]), nil
	case quackVariantTypeUint16:
		return int64(le16(ptr)), nil
	case quackVariantTypeUint32:
		return int64(le32(ptr)), nil
	case quackVariantTypeUint64:
		// Unlike a top-level UBIGINT column (quackLogicalTypeUBigInt's own
	...
	case quackVariantTypeUint128:
		return quackHugeintBigInt(ptr[:16], false).String(), nil
	case quackVariantTypeFloat:
		return float64(math.Float32frombits(le32(ptr))), nil
	case quackVariantTypeDouble:
		return le64ToFloat64(le64(ptr)), nil
	case quackVariantTypeDecimal:
		width, n1, err := quackReadVarint(ptr, 0)
		if err != nil {
			return nil, fmt.Errorf("decimal width: %w", err)
		}
		scale, n2, err := quackReadVarint(ptr, n1)
		if err != nil {
			return nil, fmt.Errorf("decimal scale: %w", err)
		}
		valBytes := ptr[n2:]
		var v *big.Int
		switch quackDecimalPhysicalWidth(byte(width)) {
		case 2:
		...
		default:
			v = quackHugeintBigInt(valBytes[:16], true)
		}
		return quackDecimalToString(v, byte(scale)), nil
	case quackVariantTypeVarchar:
		length, n, err := quackReadVarint(ptr, 0)
		if err != nil {
	...
	case quackVariantTypeUUID:
		return quackUUIDToString(ptr[:16]), nil
	case quackVariantTypeDate:
		return quackDateToTime(int32(le32(ptr))), nil
	case quackVariantTypeTimeMicros:
		return quackTimeToTime(int64(le64(ptr))), nil
	case quackVariantTypeTimeMicrosTZ:
		return quackTimeTZToTime(le64(ptr)), nil
	case quackVariantTypeTimestampSec:
		return quackTimestampToTime(quackLogicalTypeTimestampSec, int64(le64(ptr))), nil
	case quackVariantTypeTimestampMillis:
		return quackTimestampToTime(quackLogicalTypeTimestampMs, int64(le64(ptr))), nil
	case quackVariantTypeTimestampMicros:
		return quackTimestampToTime(quackLogicalTypeTimestamp, int64(le64(ptr))), nil
	case quackVariantTypeTimestampNanos:
		return quackTimestampToTime(quackLogicalTypeTimestampNs, int64(le64(ptr))), nil
	case quackVariantTypeTimestampMicrosTZ:
		return quackTimestampToTime(quackLogicalTypeTimestampTZ, int64(le64(ptr))), nil
	case quackVariantTypeInterval:
		months := int32(le32(ptr[0:4]))
		days := int32(le32(ptr[4:8]))
		micros := int64(le64(ptr[8:16]))
		return quackIntervalToString(months, days, micros), nil
	case quackVariantTypeObject:
		childCount, childrenIdx, err := decodeQuackVariantNestedData(ptr)
		if err != nil {
			return nil, err
		}
		...
		for i := 0; i < childCount; i++ {
			...
			val, verr := decodeQuackVariantValue(keys, children, values, data, int(valuesIdx))
			if verr != nil {
				return nil, verr
			}
			m[key] = val
...
// decodeQuackVariantNestedData decodes an OBJECT/ARRAY value's nested data
...
func decodeQuackVariantNestedData(ptr []byte) (childCount int, childrenIdx int, err error) {
	count, n, err := quackReadVarint(ptr, 0)
	if err != nil {
	...
	idx, _, err := quackReadVarint(ptr, n)
	if err != nil {
...
// quackReadVarint decodes one standard unsigned LEB128 varint (duckdb's
// VarintDecode/VarintEncode, serializer/varint.hpp — the exact same
// encoding quackReader.readUnsignedLeb128 reads off the wire, just from an
...
// following it.
func quackReadVarint(b []byte, offset int) (value uint64, newOffset int, err error) {
	shift := 0
	for {
		if offset >= len(b) {
