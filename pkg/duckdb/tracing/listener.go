...
func (l *listener) OnEventReceived(ctx context.Context, evt event.TrackedEvent) {
	event := evt.GetEvent()
	// A nil Data marshals to the JSON literal null, not {} — inngest.events'
	// event_data is NOT NULL DEFAULT '{}', but that default only applies to
	// a column omitted from the INSERT entirely, never to an explicit null
