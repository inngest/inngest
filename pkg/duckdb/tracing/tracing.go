...
// runsMaterializingSpanNames are the only span names materializeRuns
// promotes into a row in inngest.runs.
var runsMaterializingSpanNames = []string{"executor.run", "executor.run.queued", "executor.run.started"}
...
// it (l.tp.CreateSpan(...)) with the same CreateSpanOptions/Seed/Parent
// plumbing executor.go's own e.tracerProvider uses, getting identity
// computation and Seed/Parent semantics from the one real implementation
// rather than a second, hand-maintained one.
//
// It uses pkg/tracing/v3 rather than pkg/tracing's own TracerProvider: that
// package's executionProcessor leans on ambient, context-derived state this
// package has no way to read, so every hook below sets the equivalent
// attributes explicitly instead (see addTenantAndDebugAttrs, called from
// l.createSpan).
//
// It's a separate TracerProvider from the process's real one (pkg/devserver
// wires that one straight to sqlite) because which spans this package
// emits, and with what attributes, is expected to diverge over time — see
// SpanExporter's doc comment.
...
// Metadata set — see AddMetadataTenantAttrs), since this package's
// TracerProvider doesn't run that processor and would otherwise drop these
// from every span it creates.
func addTenantAndDebugAttrs(attrs *meta.SerializableAttrs, md *sv2.Metadata) {
	...
	// meta.AddAttr(attrs, meta.Attrs.AppName, inngestgo.Ptr(md.Config.AppName()))
	meta.AddAttr(attrs, meta.Attrs.FunctionSlug, new(md.Config.FunctionSlug()))
}

// addQueueItemAttrs sets GroupID from a queue.Item on every span that has
// one available — see addTenantAndDebugAttrs' doc comment for why this
// package sets it explicitly rather than relying on ambient state. Called
// from createSpan for every hook that sets CreateSpanOptions.QueueItem, so
// no per-hook wiring is needed.
...
// case for OnFunctionFinished's span, the only hook in this package that
// uses that name: FunctionVersion, EventIDs, CronSchedule,
// BatchID/BatchTimestamp, and a QueuedAt fallback from the run ID's own
// embedded timestamp.
...
func spanExportRow(ctx context.Context, span sdktrace.ReadOnlySpan) (row map[string]any, ok bool) {
	...
	var accountID, envID, appID, appName, functionID, functionSlug, runID string
	...
	for _, attr := range span.Attributes() {
		...
		case meta.Attrs.AppName.Key():
			appName = attr.Value.AsString()
		case meta.Attrs.FunctionID.Key():
			functionID = attr.Value.AsString()
		case meta.Attrs.FunctionSlug.Key():
			functionSlug = attr.Value.AsString()
...
func materializeRuns(ctx context.Context, db *sql.DB, rows []map[string]any) error {
	...
	query := fmt.Sprintf(`INSERT INTO inngest.runs
SELECT
  account_id,
  env_id,
  run_id,
  ...
  function_id,
  function_slug,
  attributes."_inngest.dynamic.status"::VARCHAR AS status,
  attributes,
  input AS inputs,
  output,
...
AND name IN ('executor.run', 'executor.run.queued', 'executor.run.started');`,
	...
	_, err := db.ExecContext(ctx, query, spanIDs...)
	return err
}
