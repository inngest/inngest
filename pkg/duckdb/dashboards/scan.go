...
// actual transport-specific conversion to driver.AsTimestamp -- the same
// helper pkg/cqrs/duckdbquery's own scan.go uses.
func asTime(v any, col string) (time.Time, error) {
	ts, err := driver.AsTimestamp(v)
	if err != nil {
...
// deferring to driver.AsInt64 for the actual transport-specific
// conversion.
func asInt64(v any, col string) (int64, error) {
	n, err := driver.AsInt64(v)
	if err != nil {
