...
func seedSpanRow(t *testing.T, ctx context.Context, m *Manager, accountID, envID, appID, functionID uuid.UUID, runIDULID ulid.ULID, spanID, parentSpanID, name string, start, end time.Time, attrs string) {
	...
	_, err := m.db.ExecContext(ctx,
		`INSERT INTO inngest.run_trace_spans
		 ...
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`,
		accountID.String(), envID.String(), runIDULID.String(), start, appID.String(), "test-app", functionID.String(), "test-fn",
		name, start, end, "trace-1", spanID, parentSpanID, json.RawMessage(attrs),
	)
	require.NoError(t, err)
}

// seedMetadataRow inserts one inngest.run_metadata row -- note app_id/
...
func seedMetadataRow(t *testing.T, ctx context.Context, m *Manager, accountID, envID, _, _ uuid.UUID, runID ulid.ULID, spanID, scope, kind string, isUser bool, values string, createdAt time.Time) {
	...
	_, err := m.db.ExecContext(ctx,
		`INSERT INTO inngest.run_metadata
		 ...
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`,
		accountID.String(), envID.String(), runID.String(), createdAt,
		spanID, scope, kind, isUser, json.RawMessage(values), createdAt,
	)
	require.NoError(t, err)
}
...
func TestGetSpansByRunIDSetsOutputIDWhenSpanHasOutputOrInput(t *testing.T) {
	...
	_, err := m.db.ExecContext(ctx,
		`INSERT INTO inngest.run_trace_spans
		 (account_id, env_id, run_id, run_queued_at, app_id, app_name, function_id, function_slug, name, start_time, end_time, trace_id, span_id, parent_span_id, attributes, output)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'executor.step', ?, ?, 'trace-1', 'output-span', 'root-span', '{}', ?);`,
		uuid.New().String(), uuid.New().String(), runID.String(), now, appID.String(), "test-app", functionID.String(), "test-fn",
		now.Add(time.Millisecond), now.Add(500*time.Millisecond), json.RawMessage(`{"data":"done"}`),
	)
	require.NoError(t, err)
...
func TestGetSpanOutputUnwrapsDataEnvelope(t *testing.T) {
	...
	_, err := m.db.ExecContext(ctx,
		`INSERT INTO inngest.run_trace_spans
		 (account_id, env_id, run_id, run_queued_at, app_id, app_name, function_id, function_slug, name, start_time, end_time, trace_id, span_id, attributes, output, input)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'executor.step', ?, ?, 'trace-1', 'span-1', '{}', ?, ?);`,
		uuid.New().String(), uuid.New().String(), runID.String(), now, uuid.New().String(), "test-app", uuid.New().String(), "test-fn",
		now, now, json.RawMessage(`{"data":"my-result"}`), json.RawMessage(`{"input":true}`),
	)
	require.NoError(t, err)
