...
// for every DuckDB-backed run today (see
// docs/plans/007-duckdb-gql-resolvers.md's Known gaps), so the loop never
// ran and the zero value flowed straight into models.RunTraceTrigger's
// non-nullable `timestamp: Time!` GQL field — which fails to serialize and
// crashes the whole runTrigger query with "the requested element is null
// which the schema does not allow", rather than just leaving a field
// blank.
