...
func (qr *queryResolver) Runs(ctx context.Context, num int, cur *string, order []*models.RunsV2OrderBy, filter models.RunsFilterV2, preview *bool) (*models.RunsV2Connection, error) {
	...
	for i, r := range runs {
		...
		if err != nil {
			// Preserve the prior behavior of skipping rows with parse failures
			// (e.g. unknown status, bad ulid) rather than failing the whole list.
			continue
		}

		// Track event-id → run mapping for async EventName backfill below. We
		// also need to track raw triggerIDs (already populated on node by
		// MakeFunctionRunV2) for the cross-map fan-out: skip cron/batch.
