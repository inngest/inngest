...
func (qr *queryResolver) RunTrigger(ctx context.Context, runID string) (*models.RunTraceTrigger, error) {
	...
	// DuckDB-backed runs today (see docs/plans/007-duckdb-gql-resolvers.md's
	// Known gaps: TriggerIDs isn't captured yet) — leaving ts at its zero
	// value, which fails to serialize against the non-nullable schema type
	// and crashes the whole query rather than just leaving a field blank.
